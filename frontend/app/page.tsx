...
export default function Home() {
  ...
  const handleCreatePercentageAlert = async (watchlistId: number, metricA: string, metricB: string, thresholdPct: number) => {
    try {
      await createPercentageAlert(watchlistId, metricA, metricB, thresholdPct);
      const updated = await getWatchlist();
      setWatchlist(updated);
    } catch (e: any) {
      alert(e?.response?.data?.detail || "Fehler beim Erstellen des Alerts");
  ...
  const handleDeletePercentageAlert = async (alertId: number) => {
    try {
      await deletePercentageAlert(alertId);
      const updated = await getWatchlist();
      setWatchlist(updated);
    } catch { /* ignore */ }
  };

  const handleTogglePercentageAlert = async (alertId: number) => {
    try {
      await togglePercentageAlert(alertId);
      const updated = await getWatchlist();
      setWatchlist(updated);
    } catch { /* ignore */ }
  };
  ...
  const watchlistQueries = watchlist.map(w => w.query);
  const currentWatchlistIndex = selectedItemForChart
  ...
  // Find the watchlist item for the currently selected chart item
  const currentWatchlistItem = watchlist.find(w =>
            ...
            <div className="inline-flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded text-sm transition-colors ${upgradeFilter === filter.id
          ...
          <div className="md:col-span-3 bg-slate-800 rounded-lg border border-slate-700">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {telegramSettings && (
                  <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${telegramSettings.is_active ? 'bg-emerald-600/20 text-emerald-400' : 'bg-red-600/20 text-red-400'
                    }`}>
                ...
                {telegramSettings && (
                  <div className="flex items-center gap-3 pt-2">
                    <button
                      ...
                      className={`px-4 py-1.5 rounded text-sm font-medium transition-colors flex items-center gap-2 ${telegramSettings.is_active
                        ...
                        {watchlist.map(item => (
                          <React.Fragment key={item.id}>
                            <tr className={`border-b border-slate-700/50 ${item.is_active ? '' : 'opacity-50'}`}>
                              ...
                              <td className="py-2 px-3 text-center">
                                <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${item.is_active
